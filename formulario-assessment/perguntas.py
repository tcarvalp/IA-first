# Banco de perguntas do Assessment - Itens 5, 6 e 7

# =============================================================================
# ITEM 5 - Engenheiro de Qualidade
# =============================================================================
...
PERGUNTAS_QUALIDADE = {
    "5.1 Processos de Qualidade": [
        ...
        "É realizada análise de causa raiz (RCA) para defeitos críticos?",
    ...
    "5.2 Automação de Testes": [
        ...
        "Existe estratégia para manter a saúde da suíte ao longo do tempo?",
    ],
    ...
    "5.4 Métricas de Qualidade": [
        ...
        "Existem valores-alvo (targets) definidos para as métricas no TO BE?",
    ...
    "5.5 Processos e Documentação": [
        ...
        "Como é feita a gestão do ambiente de homologação? (acesso, massa, disponibilidade)",
    ...
    "5.6 Compliance e Regulatório": [
        ...
        "Como é validada a conformidade no compartilhamento de dados de extrato?",
        "Existem testes de consentimento e revogação de compartilhamento de dados?",
        "Os dados compartilhados via Open Finance são consistentes com o extrato dos canais próprios?",
    ...
    "5.7 Testes Não Funcionais — Performance": [
        ...
        "Quais tipos de teste de performance são executados? (carga, estresse, pico, endurance, escalabilidade)",
        "Quais ferramentas são utilizadas? (JMeter, Gatling, K6, Locust, Artillery, NeoLoad)",
        "Existe baseline de performance definida? (tempo de resposta aceitável, throughput mínimo)",
        "Os testes de performance são executados em ambiente dedicado ou compartilhado?",
        "Existe monitoramento de performance no mobile? (renderização, memória, CPU, bateria)",
        "Como são definidos os SLAs de performance para o extrato?",
    ...
    "5.8 Testes Não Funcionais — Data Test (Qualidade de Dados)": [
        ...
        "Como é garantida a integridade dos dados do extrato? (consistência core banking ↔ APP)",
        "Existem testes de validação de dados ponta a ponta? (origem → API → APP)",
        "Existe monitoramento de data quality em produção? (dados ausentes, duplicados, inconsistentes)",
        "Como é validada a precisão dos valores monetários? (casas decimais, arredondamento, moeda)",
        "Existem testes de migração de dados?",
        "Existe validação de dados em cenários de alta volumetria?",
        "Como é tratada a paginação e lazy loading com grandes volumes?",
        "Existe teste de reconciliação entre APP vs. portal web vs. core banking?",
    ...
    "5.9 Testes Não Funcionais — Segurança": [
        ...
        "São testados cenários de vazamento de dados sensíveis? (logs, cache, screenshots)",
        "Existe validação de criptografia em trânsito e em repouso?",
        "Testes de certificate pinning são realizados?",
        "Como é testada a proteção contra scraping/automação maliciosa?",
    ],
    "5.10 Testes Não Funcionais — Acessibilidade": [
        "Existe estratégia de testes de acessibilidade?",
        "Quais guidelines são seguidos? (WCAG 2.1, diretrizes iOS/Android)",
    ...
    "5.11 Testes Não Funcionais — Usabilidade e Compatibilidade": [
        ...
        "Como é testada a compatibilidade com diferentes resoluções e telas?",
        ...
        "São testados cenários de interrupção? (chamada, notificação, multitasking)",
    ],
    "5.12 Testes Não Funcionais — Confiabilidade e Resiliência": [
        ...
        "Como é validado o comportamento do APP em cenários de erro? (mensagens, retry, fallback)",
    ...
    "5.13 Observabilidade e Monitoramento (visão QA)": [
        "QA valida que os logs e traces estão sendo gerados corretamente?",
        "Existe teste de alertas? (o alerta dispara quando o SLO é violado?)",
        ...
        "QA acompanha métricas e alertas nas primeiras horas após o release?",
    ...
    "5.14 Processo de Desenvolvimento (visão QA)": [
        ...
        "Existe prática de shift-right / testing in production?",
        "QA valida releases canary?",
        "QA valida o comportamento sob feature flags? (ligada/desligada, rollout progressivo)",
        "QA participa da validação de testes A/B?",
        "O monitoramento pós-release é tratado formalmente como etapa de QA?",
    ],
    "5.15 Time e Capacitação de QA": [
        ...
        "Existe skills matrix do time de QA?",
        "Os QAs possuem certificações relevantes? (CTFL/ISTQB, ferramentas, cloud)",
        "Existe plano de treinamento e capacitação contínua?",
        "Existe risco de conhecimento tribal? (bus factor)",
        "O conhecimento de QA está documentado e compartilhado?",
    ...
    "5.16 Test Management e Rastreabilidade": [
        ...
        "É possível rastrear quais requisitos não possuem casos de teste?",
        "As execuções de teste ficam registradas com evidências vinculadas?",
        "A ferramenta de gestão de testes está integrada ao Jira/backlog?",
        "Existe organização dos casos de teste por suítes/features/regressão?",
    ],
    "5.17 Cobertura Funcional do Extrato (Regras de Negócio)": [
        "Filtros por período são testados? (dia, semana, mês, customizado)",
        ...
        "O cálculo e a exibição do saldo são testados? (disponível, bloqueado, atual)",
        "Lançamentos futuros/agendados são testados?",
        ...
        "A ordenação e o agrupamento de transações são testados?",
    ...
    "5.18 Estratégia de Teste do Portal Web": [
        ...
        "São realizados testes de responsividade web?",
        "Existem testes cross-channel? (jornada mobile ↔ web)",
        "A consistência de dados entre canais é validada?",
        "A consistência de experiência entre canais é validada?",
        "Existe automação de testes web? (Cypress, Playwright, Selenium)",
        "Como é garantida a paridade funcional entre portal web e APP mobile?",
    ],
}

# =============================================================================
# ITEM 6 - Engenheiro de Desenvolvimento
# =============================================================================

PERGUNTAS_DESENVOLVIMENTO = {
...
# =============================================================================
# ITEM 7 - Compartilhados (Engenheiro de Qualidade + Engenheiro de Desenvolvimento)
# =============================================================================

PERGUNTAS_COMPARTILHADAS = {
